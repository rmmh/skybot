...
def db_init(db):
    db.execute(
        "create table if not exists memory(chan, word, data, nick,"
...
def get_memory(db, chan, word):
    row = db.execute(
        "select data from memory where chan=? and word=lower(?)", (chan, word)
    ).fetchone()
    if row:
        return row[0]
    else:
...
def remember(inp, nick="", chan="", db=None):
    ...
    if data:
        if append:
            ...
            return 'forgetting "%s", remembering this instead.' % data.replace(
                '"', "''"
            )
    else:
        return "done."
...
message_len_limit = (
    512 - len(more_pages_message) - 75
)  # fudge factor for protocol overhead
...
class MemoryTest(unittest.TestCase):
    ...
    def question(self, inp, chan="#test"):
        output = []
        question(
            re.match(r"(\S+) ?(\d+)?", inp), chan=chan, say=output.append, db=self.db
        )
    ...
    def test_get_page(self):
        ...
        assert get_page("123,456789", 0, 5, 8) == (
            "123,4567",
            8,
        )  # min length not satisfied
        assert get_page("123,45,67,89", 0, 5, 8) == (
            "123,45",
            6,
        )  # chooses correct comma
    ...
    def test_paging_message(self):
        ...
        assert more_pages_message % 2 in self.question(
            "long", chan="#long"
        )  # "2 pages left"
    ...
    def test_strip_leading_comma(self):
        long_string = (
            "long "
            + "x" * (message_len_limit - 10)
            + ", "
            + "y" * (message_len_limit - 10)
        )
