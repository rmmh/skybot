...
def add_quote(db, chan, nick, add_nick, msg):
    db.execute(
        """insert or fail into quote (chan, nick, add_nick,
                    msg, time) values(?,?,?,?,?)""",
...
def get_quotes_by_nick(db, chan, nick):
    return db.execute(
        "select time, nick, msg from quote where deleted!=1 "
...
def get_quote_by_id(db, num):
    return db.execute(
        "select time, nick, msg from quote where deleted!=1 " "and rowid=?", (num,)
    ).fetchall()
...
def quote(inp, nick="", chan="", db=None, admin=False):
    ".q/.quote [#chan] [nick] [#n]/.quote add|delete <nick> <msg> -- gets " "random or [#n]th quote by <nick> or from <#chan>/adds or deletes " "quote"
    ...
    if num:
        if num > n_quotes or (num < 0 and num < -n_quotes):
            return "I only have %d quote%s for %s" % (
                n_quotes,
                ("s", "")[n_quotes == 1],
                select,
            )
...
class QuoteTest(unittest.TestCase):
    ...
    def add_quote(self, msg=""):
        add_quote(
            ...
            "Plato",
            msg
            or "Education is the kindling of a flame," " not the filling of a vessel.",
        )
    ...
    def test_del_not_exists(self):
        assert "found no matching" in self.quote("delete whoever 4", admin=True)

    def test_del(self):
        self.add_quote("hi")
        assert "deleted quote 'hi'" in self.quote("delete socrates hi", admin=True)
