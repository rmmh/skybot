...
def ethereum(inp, say=None):
    ".ethereum -- gets current exchange rate for ethereum from BTC-e"
    data = http.get_json("https://btc-e.com/api/2/eth_usd/ticker")
    say("USD/ETH: \x0307{buy:.0f}\x0f - High: \x0307{high:.0f}\x0f"
            " - Low: \x0307{low:.0f}\x0f - Volume: {vol_cur:.0f}".format(**data['ticker']))
